...
async fn wait_to_socket() {
  ...
  let h1 = tokio::spawn(async move {
    ...
    let buf = b"GET / HTTP/1.1\r\nContent-type: text/plain\r\n\r\nthis is really random and a really big ass request body without a eof\r\n";

    let n = write.write_all(buf).await?;

    dbg!("written : {}", n);
    dbg!(buf.last());
    // write.write(b"host:3000").await?;
